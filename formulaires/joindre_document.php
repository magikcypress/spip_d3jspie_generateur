...
function joindre_determiner_mode($mode,$id_document,$objet){
	if ($mode=='auto'){
		if (intval($id_document))
			$mode = sql_getfetsel('mode','spip_d3jspie_generateur_documents','id_document='.intval($id_document));
...
function formulaires_joindre_document_charger_dist($id_document='new',$id_objet=0,$objet='',$mode = 'auto',$galerie = false, $proposer_media=true, $proposer_ftp=true){
	...
	spip_log($mode, 'test.' . _LOG_ERREUR);
			...
			if (!count($files))
				...
				foreach($files as $file){
					if (isset($file['error'])
						...
						spip_log($test, 'test.' . _LOG_ERREUR);
							if (is_string($test))
	...
	foreach ($fichiers as $f) {
		...
		if (preg_match(",\.([^.]+)$,", $f, $match)) {
			...
			if (!isset($exts[$ext])) {
				...
				if (sql_fetsel('extension', 'spip_d3jspie_generateur_documents', $a = "extension='$ext'" . $inclus))
					$exts[$ext] = 'oui';
				else $exts[$ext] = 'non';
			}
